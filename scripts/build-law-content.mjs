...
function polishText(text) {
  ...
  const OCR_FIX = [
    ...
    ["《北齐体》", "《北齐律》"],
    ["恪守职业道宿", "恪守职业道德"],
    // 人/入 OCR 混淆（巡逻轮1 全库核实 13 对，过人/记录人/翻译人等合法词不含在内）
    ["进人", "进入"],
    ["收人", "收入"],
    ["深人", "深入"],
    ["加人", "加入"],
  ...
  .replace(/(?<!之)一《/g, "→《")
  // 真题年份标注丢左括号（步首"2018年、2006年非法学案例…简答）一般累犯…"）：
  // 步首年份到闭括号之间只含年份/题型词才补（防"1689年《权利法案》等判例（1）"误伤）
  .replace(/^(\d{4}年[0-9、，\s年月日非法学简答论述分析案例辨析]{0,70}?[)）])/, "（$1");
