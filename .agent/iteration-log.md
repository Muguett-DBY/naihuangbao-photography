...
## Campaign 018 Stage 4 — Editor Model Loading Resilience

### 承接的上一轮方向
- Stage 3 已完成损坏图片恢复体验，下一风险是人脸模型请求卡住后编辑器长期停留在加载态。
- 本阶段按 IMPROVE 加固模型加载超时、重试、降级和旧请求竞态边界。

### 完成内容
- 给 face-api 动态导入、后端准备和两个模型文件加载组成的完整链路增加 20 秒截止时间。
- 将结果区分为普通失败与超时，超时时显示专用提示，手动滤镜、文字、边框和导出继续可用。
- 新增 `modelLoadRequestRef` 请求序号，旧请求的进度、结果和清理不会覆盖新重试。
- 失败后释放缓存 Promise，允许重新上传或点击重试直接发起新请求。
- 区分首次加载与重试状态，并补齐 zh-CN/en/ja/ko 文案。
- 增加超时工具单测、源码契约测试和模型请求失败后的浏览器降级/导出用例。

### 已通过的验证
- Red/green：新增测试先出现 4 个预期失败，实施后目标测试 21/21 通过。
- TypeScript / lint：通过。
- Vitest 全量：50 个文件、321/321 通过。
- `build:full` + performance budget：通过。
...
- 极慢网络可能在 20 秒后进入降级模式，但用户可继续手动编辑、导出并再次重试。

### 下一轮建议方向
1. CHECK：系统检查源码、依赖、PWA、权限、安全、构建和核心流程，修复可复现问题。
2. CHECK：重点审查编辑器、预约、登录、后台和 Service Worker 的失败边界。
3. CHECK：核对 GitHub Actions 命令与本地生产验证是否一致。
...
## Campaign 018 Stage 6 — Booking Date Boundary Alignment

### 承接的上一轮方向
- Stage 5 加严了服务端预约日期校验，下一步建议让前端预约入口和服务端规则一致。
- 本阶段按 IMPROVE 收尾预约入口韧性，避免用户在 UI 中选择或提交服务端会拒绝的日期。

### 完成内容
- 新增前端业务日期工具，按 `Asia/Shanghai` 计算工作室业务日，并严格判断真实日期和可预约日期。
- 预约弹窗改用同一业务日期作为日历最小日期，进入下一步和提交前都会校验已选日期。
- 用户中心预约改期复用同一业务日期规则，过去日期会在前端直接阻止并给出错误提示。
- 日历新增“最早可预约日期”提示，过去日期 disabled 时可访问标签说明不可预约原因。
- 补齐 zh-CN/en/ja/ko 文案。
- 增加业务日期单测、源码回归和预约弹窗 E2E。

### 已通过的验证
- Red/green：业务日期工具和源码合同先失败，修复后目标测试 64/64 通过。
- 预约业务日期目标 Playwright：1/1 通过。
- TypeScript / lint：通过。
- Vitest 全量：52 个文件、333/333 通过。
- `build:full` + performance budget：通过。
...
### 下一轮建议方向
1. 抽取或下发 booking policy，让前端日历边界由 API 配置驱动，减少双端规则漂移。
2. 对登录/注册/重置等公开认证端点继续做 CSRF/header 策略审计。
3. 继续处理 PWA 旧 service worker 更新体验，降低 Browser 和真实用户看到旧 bundle 的概率。
...
### GitHub Actions / CI 状态
- Stage 6 commit `c23dc77` 已 push 到 `main`。
- GitHub Actions CI run `28406599343` 通过。
...
## Campaign 020 Stage 1 — PWA Install Flow IMPROVE

### 承接的上一轮方向
- Campaign 019 已完成 PWA 更新发现与接管可靠性；本阶段继续 PWA 产品线，但转向独立的安装提示路径。
- 现有安装提示使用 `sessionStorage` 计数，在单页应用根组件通常只挂载一次的情况下很难达到二次访问阈值；并且 `beforeinstallprompt` 事件内返回的 timer cleanup 不会被浏览器调用。

### 完成内容
- `PwaInstallBanner` 改为使用 `localStorage` 持久记录访问次数、安装完成状态和 7 天后过期的稍后提醒时间。
- 补齐 standalone / 已安装状态判断，避免已安装用户继续看到安装提示。
- 将安装提示 timer 存在组件级 ref 中，卸载、dismiss、安装完成时都会清理。
- 点击安装后等待浏览器 `userChoice`，接受安装才记录 installed；取消安装则进入有限期冷却，不再永久隐藏。
- 增加安装中、错误、稍后再说的可见状态，并补齐响应式样式。
- 四个 locale 补齐新文案键。
- 新增源码回归和 Playwright PWA 安装流测试，覆盖跨访问资格、提示出现、安装中状态、接受安装后本地状态写入。

### 已通过的验证
- Red/green：目标 PWA 安装回归测试先失败，修复后通过。
- `npm test -- src/lib/pwa-install.test.ts src/lib/audit-regressions.test.ts`：2 个文件、71 个测试通过。
- TypeScript / lint：通过。
- Vitest 全量：56 个文件、348 个测试通过。
- `build:full` + performance budget：通过。
- Playwright 全量：35/35 通过。

### GitHub Actions / CI 状态
- Stage 1 commit `c876808` 已 push 到 `main`。
- GitHub Actions CI run `28408823208` 通过。

### 遗留风险
- 浏览器是否触发 `beforeinstallprompt` 仍受各浏览器 PWA 安装启发式规则控制；本阶段修复的是应用侧资格、冷却、结果等待和 UI 状态。

### 下一轮建议方向
1. IMPROVE：选择一个不重复 PWA 更新/安装路径的可靠性改进点。
2. 保持小范围、可回归、可浏览器验证，避免在同一 campaign 里引入大面积架构迁移。

### 推荐下一轮优先执行的旗舰级主改动
围绕预约/候补/用户状态中仍缺少直接回归覆盖的小边界做一次可靠性改进。

---

## Campaign 020 Stage 2 — Waitlist Idempotency IMPROVE

### 承接的上一轮方向
- Stage 1 建议下一轮围绕预约/候补/用户状态中缺少直接回归覆盖的小边界做可靠性改进。
- 本阶段选择满员日期候补重复提交作为主线：直接预约已有重复保护，但候补路径此前只限制单日总量。

### 完成内容
- 候补 API 在 insert 前按日期和联系方式查找 active 候补记录。
- 重复候补返回 `already_waitlisted` 幂等成功和 `duplicate: true`，不再创建重复行。
- 重复候补响应不暴露既有 unsubscribe token，避免仅凭联系方式和日期取到退订 token。
- 候补 API 写入前统一 trim 日期、联系方式和姓名。
- 预约弹窗新增“已在候补名单中”的成功态，而不是显示普通错误或误导为新建候补。
- 四语言补齐重复候补文案，并给已存在候补状态加轻量视觉区分。

### 已通过的验证
- Red/green：候补 API + 源码回归先失败，修复后 2 个文件、70 个测试通过。
- TypeScript / lint：通过。
- Vitest 全量：56 个文件、350 个测试通过。
- `build:full` + performance budget：通过。
- Playwright 新增重复候补用例：1/1 通过。
- Playwright 全量：36/36 通过。
- In-app Browser 本地 preview：页面非空、主标题可见、无框架错误覆盖、console error/warn 为空。

### GitHub Actions / CI 状态
- Stage 2 commit `28fd870` 已 push 到 `main`。
- GitHub Actions CI run `28409731031` 通过。

### 遗留风险
- 当前是应用层幂等保护；极端并发的完全相同候补请求未来可在生产数据去重后补 DB-level uniqueness guard。

### 下一轮建议方向
1. UIUX：继续在预约或 dashboard 里做用户明显感知的状态说明和响应式体验提升。
2. UIUX：优先减少预约完成后用户对“下一步/是否已保存/何时联系”的不确定感。

### 推荐下一轮优先执行的旗舰级主改动
预约与个人中心之间的状态可见性：让用户从提交、候补、查看预约到后续跟进的路径更清楚、更少迷路。

---

## Campaign 020 Stage 3 — Booking Completion UIUX

### 承接的上一轮方向
- Stage 2 已把重复候补提交改成幂等成功态。
- 本阶段按 UIUX 主线，继续改善用户从预约/候补完成态进入后续跟进路径的清晰度。

### 完成内容
- 普通预约成功态和候补成功态共用新的 `booking-success-bridge` 下一步操作区。
- 将原先裸露的 Xiaohongshu URL 和松散按钮改为三个明确动作：查看我的预约、小红书私信、继续浏览。
- 普通预约与候补状态都保留 reference/date/package 等确认信息，并在其下方提供后续动作。
- 操作区加入 Lucide 图标、可聚焦样式、移动端单列布局和桌面三列布局。
- zh-CN/en/ja/ko 补齐新文案。
- 新增源码契约和 Playwright 断言，覆盖移动端定金成功态与桌面满员候补成功态。

### 已通过的验证
- Red/green：`npm test -- src/lib/audit-regressions.test.ts` 先因缺少完成态桥接区失败，修复后 68/68 通过。
- `npm run build`：通过，并用于刷新 Playwright 预览产物。
- 目标预约 Playwright：2/2 通过，覆盖移动端定金成功态和桌面候补成功态。
- TypeScript / lint：通过。
- Vitest 全量：56 个文件、351 个测试通过。
- `build:full` + performance budget：通过。
- Playwright 全量：36/36 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。

### GitHub Actions / CI 状态
- Stage 3 commit `b7b7b6a` 已 push 到 `main`。
- GitHub Actions CI run `28410527355` 通过。

### 遗留风险
- 未登录用户点击“查看我的预约”仍会进入既有登录边界；这是当前 dashboard 访问模型，不在本阶段改变。

### 下一轮建议方向
1. IMPROVE：选择一个预约/客户状态相关的小可靠性边界继续强化。
2. IMPROVE：优先选择能用 API、源码契约和浏览器路径一起验证的改动。
3. 避免重复扩展本次完成态 UI，把范围留给下一处独立用户价值。

### 推荐下一轮优先执行的旗舰级主改动
预约完成后的状态可靠性：在 dashboard 或通知链路里继续减少“已提交但用户不确定状态”的场景。

---

## Campaign 020 Stage 4 — Post-Booking Status Reliability IMPROVE

### 承接的上一轮方向
- Stage 3 已把预约/候补成功态接到 dashboard、私信和继续浏览的下一步操作。
- 本阶段继续处理“已提交但用户不确定状态”的可靠性问题，重点补齐成功态后进入 dashboard 与离线恢复提交的真实边界。

### 完成内容
- 修复离线预约恢复同步：`syncPendingBookings` 现在复用 `publicMutationHeaders`，与在线预约提交保持同一 public mutation 边界。
- 新增 `createPendingBookingRequestInit`，让离线恢复请求的 method、headers、payload 可被单元测试直接验证。
- 未登录访问 `/dashboard` 改为进入 `/login?from=dashboard`，保留“回来查看预约”的意图。
- 登录页新增预约状态上下文提示，说明登录后会回到个人中心查看预约更新和定金状态。
- 登录/注册成功后在安全白名单范围内回到 dashboard；未带该参数时仍回首页。
- 四语言补齐 dashboard 登录提示文案。

### 已通过的验证
- Red/green：离线恢复 header 和 dashboard 登录上下文测试先失败，修复后 2 个文件、70 个测试通过。
- 目标 login/dashboard Playwright：5/5 通过。
- TypeScript / lint：通过。
- Vitest 全量：57 个文件、353 个测试通过。
- `build:full` + performance budget：通过。
- Playwright 全量：37/37 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。

### GitHub Actions / CI 状态
- Stage 4 commit `9e1d0cb` 已 push 到 `main`。
- GitHub Actions CI run `28418534404` 通过。

### 遗留风险
- 本阶段只允许固定的 `from=dashboard` 回跳，避免 open redirect；未来如需要更多来源，应继续使用白名单而不是任意 URL。

### 下一轮建议方向
1. CHECK：围绕本轮新增的登录回跳、离线恢复提交、auth mutation 边界和 PWA 产物做一次安全/稳定性扫雷。
2. CHECK：确认本地验证命令与 GitHub Actions 仍一致，特别是 build/perf budget 与 e2e 产物路径。
3. CHECK：检查是否还有 public mutation 路径遗漏 `publicMutationHeaders` 或缺少浏览器回归。

### 推荐下一轮优先执行的旗舰级主改动
预约状态恢复 CHECK：审查 booking/auth/offline/PWA 的真实恢复链路，发现可复现风险就补回归并修复。

---

## Campaign 020 Stage 5 — Offline Booking Recovery CHECK

### 审查结论
- P0：未发现。`npm audit --json` 为 0 漏洞，`npm ls --depth=0` 正常，本地安装、lint、测试、构建命令与 GitHub Actions 保持一致。
- P1：离线预约恢复遇到永久 4xx 拒绝后仍保留 `pending`，后续每次 online 事件都会重复提交同一无效请求。
- P2：既有 `failed` 状态实际上不可达，且没有回归测试锁定 2xx/4xx/5xx 的处理策略。

### 完成内容
- 新增离线同步响应分类：2xx 标记为 `synced`，4xx 标记为终态 `failed`，5xx 和网络错误继续保留 `pending` 等待后续重试。
- 新增 `markBookingFailed`，阻止服务端已经明确拒绝的预约无限重放，同时保留本地记录用于诊断。
- 新增单元测试和源码契约，覆盖 201、409、429、503 以及失败状态写入路径。

### 已通过的验证
- Red/green：目标测试先因缺少响应分类和失败状态处理而失败，修复后 2 个文件、71 个测试通过。
- `npm audit --json`：0 漏洞。
- `npm ls --depth=0`：正常。
- `npm ci`：通过，456 个包、0 漏洞。
- TypeScript / lint：通过。
- Vitest 全量：57 个文件、354 个测试通过。
- `build:full` + performance budget：通过。
- Playwright 全量：37/37 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。

### GitHub Actions / CI 状态
- Stage 5 commit `cbc2166` 已 push 到 `main`。
- GitHub Actions CI run `28419148049` 通过。

### 遗留风险
- 终态失败记录仍保留在 IndexedDB 中，当前没有用户可见入口查看或移除；这避免静默丢数据，但恢复能力仍不完整。

### 下一轮建议方向
1. IMPROVE：让离线预约的 pending/failed 状态对用户可见。
2. 提供有边界的立即重试或移除失败记录动作，避免引入新的服务端数据模型。
3. 使用单元测试、源码契约和浏览器流程验证恢复操作。

### 推荐下一轮优先执行的旗舰级主改动
离线预约恢复面板：显示本地待同步/失败预约，并提供明确、可验证的重试和清理路径。

---

## Campaign 020 Stage 6 — Offline Booking Recovery IMPROVE

### 承接的上一轮方向
- Stage 5 已修复离线预约遇到终态 4xx 后无限重试的问题，但失败记录仍保留在 IndexedDB 且用户不可见。
- 本阶段按 IMPROVE 主线补齐用户可见恢复入口：让 pending/failed 本地预约在弹窗关闭后仍能被发现、同步、重约或移除。

### 完成内容
- 新增全局懒加载 `OfflineBookingRecovery` 恢复带，挂在公共页面 Header 下方。
- 将 online 自动同步从 `BookingModal` 移到全局恢复组件，避免用户关闭弹窗后失去自动恢复能力。
- pending 本地预约在线后可立即同步；failed 本地预约显示终态失败并提供重新预约和移除。
- 预约成功同步后删除本地 IndexedDB 记录，不再留下陈旧 `synced` 记录；组件挂载时也清理历史 `synced` 记录。
- `savePendingBooking` 失败时不再显示“已离线保存”的成功提示，而是走明确错误。
- 离线/在线横幅改为四语言文案和 Lucide 状态图标。
- 补齐单元测试、源码契约测试、Playwright 流程测试和桌面/移动视觉验证。

### 已通过的验证
- Red/green：目标离线恢复测试先因缺少全局恢复契约失败，修复后 2 个文件、73 个测试通过。
- 目标 Playwright：`offline-booking-recovery.spec.ts` 2/2 通过。
- In-app Browser 本地 preview：页面标题正确，首页可渲染，console error/warn 为空。
- 桌面 1280x900 与移动 390x844 视觉检查：恢复带位于导航下方、主内容上方，无恢复区重叠；页面无文档级横向滚动条。
- `npm ci`：首次因本轮 preview 进程占用 Rolldown 原生绑定而触发 Windows `EPERM`，停止当前项目 preview 后通过，456 个包、0 漏洞。
- TypeScript / lint：通过。
- Vitest 全量：57 个文件、356 个测试通过。
- `build:full` + performance budget：通过，恢复组件作为独立 lazy chunk 输出。
- Playwright 全量：39/39 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。

### GitHub Actions / CI 状态
- Stage 6 commit `20506ea` 已 push 到 `main`。
- GitHub Actions CI run `28420698040` 通过。

### 遗留风险
- failed 本地记录目前支持移除或重新打开预约流程，但不支持在原记录上直接编辑并重放；如后续真实用户需要，可单独做 edit-resubmit。

### 下一轮建议方向
1. Campaign 020 的 6 个阶段完成后，先观察 GitHub Actions 与线上表现。
2. 如继续迭代，建议转向更大粒度的预约时间段模型或生产数据层唯一性约束，但应作为新 campaign 处理。

---

## Campaign 020 Post-Completion Hardening Sweep

### 本轮性质
- 用户在 6 阶段完成后要求继续。
- 本轮不启动新的 6 阶段 campaign，只做追加安全/稳定性扫查：发现真实可复现问题即补测试并修复。

### 审查发现
- P1：后台照片上传前端未发送 `x-nhb-admin-action`，但 `/api/admin/photos` POST 后端要求该 header，导致上传成功路径实际会被 403 拒绝。
- P1：预设下载计数 `/api/presets/:id/download` 是公开 POST 写操作，只靠 rate limit，没有统一 public page-action header；产品页和预设详情页也未发送 `publicMutationHeaders`。
- P2：后台登出 DELETE `/api/admin/session` 会清 cookie，但没有 admin action header，容易被跨站请求触发登出。
- P1：后台照片上传成功后 API 只返回 `{ id, imageUrl }`，但前端读取 `data.photo` 并插入列表，修复 header 后会继续把 `undefined` 放入照片列表。

### 完成内容
- 为预设下载计数接口增加 `requirePublicMutationRequest`，并让产品页/预设详情页发送 `publicMutationHeaders`。
- 为后台 session DELETE 增加 `isAdminMutationRequest` 校验，并让后台登出 fetch 发送 `adminMutationHeaders`。
- 为后台照片上传 fetch 补 `adminMutationHeaders`。
- 后台照片上传成功响应改为返回完整 `photo` 对象，匹配 `AdminPhotosTab` 的列表插入逻辑。
- 增加 mutation boundary、源码契约和上传成功响应测试，覆盖上述回归。

### 已通过的验证
- Red/green：新增 boundary/source 测试先失败 4 项，修复后 `functions/mutation-boundaries.test.ts` + `src/lib/audit-regressions.test.ts` 80/80 通过。
- Red/green：后台上传成功响应测试先因 `body.photo` 缺失失败，修复后目标上传测试 2/2 通过。
- `npm audit --json`：0 漏洞。
- `npm ls --depth=0`：正常。
- TypeScript / lint：通过。
- Vitest 全量：57 个文件、360 个测试通过。
- `build:full` + performance budget：通过。
- Playwright 全量：39/39 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。

### GitHub Actions / CI 状态
- Commit `e10ab7f` 已 push 到 `main`。
- GitHub Actions CI run `28422043779` 通过。

### 遗留风险
- 真实后台上传的 R2/D1 行为仍依赖 Cloudflare 资源配置；本轮修复的是前端 header、API 响应合同和可测试边界。

---

## Campaign 021 Stage 1 — Booking Time-Slot Availability IMPROVE

### 承接的上一轮方向
- Campaign 020 完成后建议把更完整的预约时间段模型作为新 campaign；当前表单虽有上午、下午、全天偏好，但只校验整日容量。

### 完成内容
- 建立共享的上午、下午、全天冲突规则：全天占用两个半日，任一半日也会阻止全天；取消状态不计入冲突。
- 可用性 API 在日期容量之外返回逐时段状态，直接预约 API 在写入前校验时段并对冲突返回结构化 409。
- 预约弹窗显示三档时段状态、禁用不可用选项，并在切换日期后清除已失效选择。
- 四语言文案、规则测试、API 测试、源码契约和 Playwright 回归同步补齐。

### 已通过的验证
- Red/green：目标测试先因缺少时段规则/API/UI 契约失败，修复后 4 个文件、115 个测试通过。
- 目标 Playwright：预约日历场景 1/1 通过。
- In-app Browser：时段状态正常渲染，已占用上午选项禁用，无横向溢出，console error/warn 为空。
- `npm ci`：456 个包、0 漏洞。
- TypeScript / lint：通过。
- Vitest 全量：57 个文件、366 个测试通过。
- `build:full` + performance budget：通过。
- Playwright 全量：39/39 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。

### GitHub Actions / CI 状态
- Stage 1 commit `ba0f147` 已 push 到 `main`。
- GitHub Actions CI run `28436973546` 通过。

### 遗留风险
- 用户中心改期仍只更换日期，尚未选择和验证新的预约时段；下一阶段将复用本阶段共享规则补齐该路径。

### 下一轮建议方向
1. IMPROVE：让改期界面同时选择日期与时段。
2. 改期 API 在事务写入前使用相同冲突规则，并排除当前预约自身。
3. 用单元、API、源码契约和用户中心浏览器流程验证。

---

## Campaign 021 Stage 2 — Time-Aware Dashboard Rescheduling IMPROVE

### 承接的上一轮方向
- Stage 1 已把上午、下午、全天时段变成直接预约的可见可用性合同。
- 本阶段补齐用户中心改期路径：改期不再只改日期，也必须选择并校验新的时段。

### 完成内容
- 新增共享 `BookingTimeSlotPicker`，直接预约弹窗和用户中心改期共用同一套时段禁用逻辑。
- 用户中心改期面板显示当前/新日程摘要，并允许选择新的日期和时段。
- 改期请求现在提交 `preferred_time`，API 会校验时段词表、排除当前预约自身、拒绝已占用目标时段，并同时更新 `preferred_date` 与 `preferred_time`。
- 修复同日改成另一时段会被自身预约误判为冲突的问题。
- 修复直接预约中“Any/任意”在两个半日时段都被占用时仍可选择的边界。
- 补齐 zh-CN、en、ja、ko 文案，并增加 API、规则、组件、源码契约和 Playwright 覆盖。

### 已通过的验证
- Red/green：缺少改期 API/共享 picker 契约时目标测试先失败；实现后目标测试 4 个文件、118 个测试通过。
- 目标 Playwright：用户中心改期到可用下午时段场景通过 1/1，覆盖移动端无溢出、请求体带 `preferred_time`、成功 toast 含日期和时段。
- `npm ci`：456 个包，0 漏洞。
- TypeScript / lint：通过。
- Vitest 全量：58 个文件、371 个测试通过。
- CI-order `npm run build`：通过。
- CI-order `npm run perf:budget`：通过。
- `npm run build:full`：通过，含 SEO sync、sitemap、AVIF、Vite build、性能预算和 bundle analysis。
- Playwright 全量：40/40 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。
- `git diff --check` 通过；调试语句/敏感信息扫描无命中。

### GitHub Actions / CI 状态
- Stage 2 commit `ab14856` 已 push 到 `main`。
- GitHub Actions CI run `28438916217` 通过。

### 遗留风险
- 当前时段冲突保护仍是应用层查询再写入；如真实生产出现同一时段并发写入，可在清理数据后追加数据库层唯一性保护。

### 下一轮建议方向
1. UIUX：围绕用户中心改期面板做可见体验打磨。
2. 强化冲突恢复、当前/新日程对比和移动端可读性。
3. 继续用源码契约、组件/浏览器测试和 CI 验证闭环。

---

## Campaign 021 Stage 3 — Guided Reschedule UIUX

### 本轮 UI/UX 主改动
- 将用户中心改期面板从平铺表单升级为 guided schedule-change workspace。
- 用户打开改期时能看到标题、说明、日期/时段/确认三步进度、当前/新安排对比和确认状态。

### 完成内容
- 新增改期面板 header、三步进度条、日历区、时段/复核区和内联状态提示。
- 确认按钮禁用时显示“请选择不同日期或时段”，可提交时显示目标日期与时段，冲突时复用错误状态。
- 桌面/中宽卡片使用容器自适应，空间不足时自动单列，避免面板横向溢出。
- 移动端打开改期面板时隐藏底部导航，避免遮挡长日历流程。
- 补齐 zh-CN、en、ja、ko 的 guided panel、步骤和状态文案。
- 增加源码契约和 Playwright 覆盖，防止步骤、状态和移动底部导航避让回退。

### 已通过的验证
- Red/green：源码契约先因缺少 guided panel 失败，修复后 74/74 通过。
- Red/green：目标 Playwright 先因缺少 `.dashboard-reschedule-steps` 失败，修复后用户中心改期场景 1/1 通过。
- 桌面/移动 preview 渲染检查：无页面横向溢出、无面板横向溢出、console error/warn 为空，移动底部导航在改期面板打开时 hidden。
- `npm ci`：456 个包，0 漏洞。
- TypeScript / lint：通过。
- Vitest 全量：58 个文件、372 个测试通过。
- CI-order `npm run build`：通过。
- CI-order `npm run perf:budget`：通过。
- `npm run build:full`：通过，含 SEO sync、sitemap、AVIF、Vite build、性能预算和 bundle analysis。
- Playwright 全量：40/40 通过。
...
### GitHub Actions / CI 状态
- Stage 3 commit `173117c` 已 push 到 `main`。
- GitHub Actions CI run `28440697538` 通过。

### 遗留风险
- 中等宽度 dashboard 卡片会优先单列而不是强制桌面两栏；这是为了避免挤压和溢出，后续若整体 dashboard 宽度重构，可重新启用更宽的双栏体验。

### 下一轮建议方向
1. IMPROVE：继续围绕预约/改期冲突恢复做产品级可靠性提升。
2. 让服务端 `time_unavailable`、日期满员和前端状态提示有更一致的恢复路径。
3. 如果继续 UIUX，则优先优化 dashboard 卡片宽度与改期面板承载空间。

---

## Campaign 021 Stage 4 — Booking Slot Conflict Recovery IMPROVE

### 承接的上一轮方向
- Stage 3 已把 dashboard 改期体验升级为 guided workspace。
- 本阶段继续补预约可靠性：当客户提交时目标时段刚被其他预约占用，服务端不仅拒绝，还返回可操作的恢复建议。

### 完成内容
- 新增共享 `BookingTimeSlotRecovery` 合同，返回当前日期是否还能保留、原请求时段、可替代时段列表和建议时段。
- 直接预约 API 与用户中心改期 API 在 `time_unavailable` 409 中都返回 `timeSlots` 和 `recovery`。
- 直接预约弹窗收到冲突后回到时段选择步骤，显示跨步骤提示，自动切到建议时段，并保留服务端最新的冲突状态，避免日历重挂载后被旧 availability 覆盖。
- 用户中心改期冲突后保留面板打开，自动选中建议时段，更新状态提示/toast，用户可直接再次确认。
- 补齐 zh-CN、en、ja、ko 的恢复提示文案。
- 增加 API、源码契约和 Playwright 覆盖，直接预约与 dashboard 改期两条恢复路径都被真实浏览器验证。

### 已通过的验证
- Red/green：`functions/api.test.ts` + `src/lib/audit-regressions.test.ts` 先因缺少 recovery 合同失败，修复后 2 个文件、110 个测试通过。
- Targeted Playwright：直接预约恢复用例先暴露“提示回到第 1 步不可见”和“日历重挂载覆盖服务端冲突状态”问题；修复后 `-g "recovers when"` 两个恢复场景 2/2 通过。
- `npm ci`：456 个包，0 漏洞；仅有依赖 deprecation 与 allow-scripts 提示。
- TypeScript / lint：通过。
- Vitest 全量：58 个文件、372 个测试通过。
- `npm run build`：通过。
- `npm run perf:budget`：通过。
- `npm run build:full`：通过，含 SEO sync、sitemap、AVIF、Vite build、性能预算和 bundle analysis。
- Playwright 全量：42/42 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。

### GitHub Actions / CI 状态
- Stage 4 commit `00af6b7` 已 push 到 `main`。
- GitHub Actions CI run `28454599381` 通过。

### 遗留风险
- 当前恢复建议仍属于应用层“查询后写入”的冲突缓解；如真实生产出现高并发同槽写入，下一步应评估数据库层唯一性约束或事务级保护。

### 下一轮建议方向
1. CHECK：对全站代码、测试和关键用户流程做广覆盖验收。
2. 重点关注本 campaign 新增的预约、改期、恢复流程是否存在漏测、竞态或文案不一致。
3. 如 CHECK 发现问题，下一阶段 IMPROVE 直接修复并补回归。

---

## Campaign 021 Stage 5 — Full-Site CHECK

### 检查发现与修复
- P1：后台审核队列在未选中照片时显示“全部批准”，但处理函数直接返回；现已在空选择时批准全部待审核照片，存在选择时仍只处理选中项。
- P1：批量拒绝只等待多个 `fetch` resolve，没有检查 HTTP 状态，4xx/5xx 也会被当成成功并从界面移除；现已检查每个响应，任一失败时保留队列并提示错误。
- P2：Lightbox E2E 点击非交互式 `article`，并发加载发生布局变化时可能点到卡片空白；现改为等待并点击真实 `.gallery-masonry-btn`。

### 已通过的验证
- `npm audit --json`：0 漏洞；`npm ls --depth=0`：正常。
- Red/green：审核队列源码契约先失败，修复后目标文件 75/75 通过。
- Lightbox 并发复现与命中元素采样确认失败点击落在 `ARTICLE` 空白区；修复后 6 worker 压力复跑 12/12 通过。
- `npm ci`：456 个包，0 漏洞。
- TypeScript / lint：通过。
- Vitest 全量：58 个文件、373 个测试通过。
- `npm run build`、`npm run perf:budget`、`npm run build:full`：通过。
- Playwright 全量：42/42 通过。
- 已恢复 sitemap 和 `test-results` 噪声；`git diff --check` 与敏感信息/调试痕迹扫描通过。

### GitHub Actions / CI 状态
- Stage 5 commit `1bea6b9` 已 push 到 `main`。
- GitHub Actions CI run `28456390852` 通过。

### 遗留风险
- 批量拒绝仍发出多个独立 DELETE；若中途出现服务端失败，后台可能已部分删除，而前端只能保留列表并要求刷新确认。

### 下一轮建议方向
1. IMPROVE：为后台照片批处理 API 增加单请求批量删除动作。
2. 前端批量拒绝改为调用该合同，统一验证、对象存储清理和错误响应。
3. 用 API、源码契约、全量测试、构建、Playwright 和 CI 完成最终闭环。

---

## Campaign 021 Stage 6 — Server-Side Batch Photo Deletes IMPROVE

### 承接的上一轮方向
- Stage 5 已修复审核队列的“全部批准”、批量拒绝 HTTP 状态检查和 Lightbox E2E 点击目标问题。
- 本阶段处理 Stage 5 遗留风险：批量拒绝仍是多个独立 DELETE，请求中途失败时可能形成部分删除。

### 完成内容
- 新增 `deletePhotosWithConsistency`：先验证所有选中照片行和 object key 都存在，再一次性调用 R2 bulk delete，并用一条 D1 delete 删除记录。
- `/api/admin/photos/batch` 新增 `action: "delete"`，支持去重/修剪 ID、统一错误响应、`batch_delete` 审计日志、缓存失效和 `{ deleted, ids }` 返回值。
- 审核队列的批量拒绝改为先显示内联危险确认，再调用批量 API，不再发起多个单图 DELETE。
- 照片管理页的批量删除也改为调用批量 API，并只移除服务端确认删除的 ID；修复了旧逻辑在部分失败时仍从界面移除全部选中项的问题。
- 补齐 zh-CN、en、ja、ko 的待处理数、已选择数、危险确认、处理中、成功和失败文案。
- 审计日志 badge 增加 `batch_delete` 样式。
- 增加 API、源码契约和 Playwright 覆盖，验证批量删除合同与审核确认流程。

### 已通过的验证
- Red/green：`functions/api.test.ts` + `src/lib/audit-regressions.test.ts` 先因缺少 batch delete API/源码合同失败，修复后 2 个文件、114 个测试通过。
- Targeted Playwright：初次失败定位为 `vite preview` 服务旧 `dist`；重新 `npm run build` 后，审核确认批量删除流程 1/1 通过。
- `npm ci`：456 个包，0 漏洞；仅有依赖 deprecation 与 allow-scripts 提示。
- TypeScript / lint：通过。
- Vitest 全量：58 个文件、376 个测试通过。
- `npm run build:full`：通过，含 SEO sync、sitemap、AVIF、Vite build、性能预算和 bundle analysis。
- Playwright 全量：43/43 通过。
- 已恢复 sitemap 和 `test-results` 噪声，未纳入提交。
- `git diff --check` 通过；仅报告两个已触碰文件的 CRLF 归一化提示。

### GitHub Actions / CI 状态
- Stage 6 commit `3ec4be8` 已 push 到 `main`。
- GitHub Actions CI run `28458867674` 通过。

### 遗留风险
- R2 与 D1 是两个独立系统，无法在同一个本地事务中保证跨服务原子性；新合同已通过“先验证全量行、再一次 R2 删除、再一次 D1 删除”显著缩小原先多请求部分删除风险。

### 总控状态
- Campaign 021 的 6 个阶段 `IMPROVE -> IMPROVE -> UIUX -> IMPROVE -> CHECK -> IMPROVE` 均已完成。
- 每个阶段都已完成实际修改、本地验证、提交、推送和 GitHub Actions 确认。
