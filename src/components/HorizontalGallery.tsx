...
import { useSwiperDistortion } from "../hooks/useSwiperDistortion";
import "swiper/css";
        ...
        >
          {items.map((item, i) => (
            <SwiperSlide key={`${item.id}-${i}`} className="horiz-swiper-slide">
              <div
                className="horiz-swiper-card"
                data-disp-img={item.imageUrl}
              >
                <img
                  src={item.imageUrl}
                  alt={item.alt}
