...
describe("Visual OS V7 worlds", () => {
  it("ships three distinct worlds backed by sixteen stable no-person assets", () => {
    expect(visualWorlds.map((world) => world.id)).toEqual(["dawn", "rain", "afterglow"]);
    expect(visualWorlds.every((world) => world.frames.length === 6)).toBe(true);
    expect(visualWorldAssets).toHaveLength(16);
    expect(new Set(visualWorldAssets.map((asset) => asset.assetId)).size).toBe(16);
  });
  ...
  it("keeps generated world imagery out of the real portfolio source", () => {
    expect(galleryItems.every((item) => !item.imageUrl.includes("visual-os-v7"))).toBe(true);
  });
});
