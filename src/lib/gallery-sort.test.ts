...
const read = (path: string) => [path, ...(path === "src/components/Gallery.tsx" ? ["src/features/gallery/GalleryCommandCenter.tsx", "src/features/gallery/gallery-discovery.ts"] : [])].map((file) => readFileSync(resolve(root, file), "utf8")).join("\n");
